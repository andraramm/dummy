(function ($) {
    ...
    $(document).on('change','#manual',function(){
        $.ajax({
            url: site_url + '/deposit/getDetailPaket/' + $('#manual').val(),
            type: 'get',
            success: function(hasil){
                var obj = $.parseJSON(hasil);
                $('#nom1').val(parseInt( obj.harga ).toLocaleString().replace(',', '.'));
            }
        })
    });

    $(document).on('change','#otomatis',function(){
        $.ajax({
            url: site_url + '/deposit/getDetailPaket/' + $('#otomatis').val(),
            type: 'get',
            success: function(hasil){
                var obj = $.parseJSON(hasil);
                $('#nom2').val(parseInt( obj.harga ).toLocaleString().replace(',', '.'));
            }
        })
    });
        ...
        counter.forEach(function (counter_value) {
            function updateCount() {
                ...
                if (count < target) {
                    ...
                    counter_value.innerText = target.toLocaleString().replace(/,/g, '.');
                }
            };
            updateCount();
        });
